...
function	exec(){
	...
	function	graph_init( width, height )	{
		...
		var	zoom = d3.zoom( "zoom", function(d) {
	...
	function	graph_make_node( collocation )	{
		...
		for( var i = 0; i < collocation.getCount(); i++)
		{
			...
			nodes.push( { name : name, no : i, is_node : 0, is_link : 0
				 , group: collocation.node_group_id[id][0]} ) ;
		}
		return nodes ;
	}
	...
	function node_color(group_id) {
		var colors = ['#008b8b', '#4682b4', '#ff7f50', '#ff8c00', '#8b008b', '#8b0000', '#b0c4de', '#a9a9a9', '#ffc0cb', '#f0e68c'] ; ;
...
//			force.resume();
		...
		node0.append("circle")
			...
			.attr("fill-opacity", function(d){return node_color(d.group);})
